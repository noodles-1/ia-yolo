...
def convert_voc_annotation(data_path, anno_path, use_difficult_bbox=False):
    ...
    img_inds_file = os.path.join(data_path, 'JPEGImages')
    image_inds = []
    for file_name in listdir(img_inds_file):
        image_inds.append(file_name[:len(file_name)-4])
...
if __name__ == '__main__':
    ...
    num1 = convert_voc_annotation(os.path.join(flags.data_path, 'voctrain'), flags.train_annotation, False)
    num2 = convert_voc_annotation(os.path.join(flags.data_path, 'vocval'), flags.val_annotation, False)
    num3 = convert_voc_annotation(os.path.join(flags.data_path, 'voctest'), flags.test_annotation, False)
